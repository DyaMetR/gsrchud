...
if CLIENT then
  ...
  local spriteData = { -- Sprite data
    ...
    ["bucket6"] = {x = 228, y = 72, w = 20, h = 20, file = file},
  ...
  function script()
    if (IsValid(LocalPlayer():GetActiveWeapon())) then
        GSRCHUD:SetCustomNumberCrit("clip", LocalPlayer():GetActiveWeapon():Clip1() < 10);
        GSRCHUD:SetCustomNumberCrit("reserve", LocalPlayer():GetAmmoCount(LocalPlayer():GetActiveWeapon():GetPrimaryAmmoType()) < 10);
    end
    GSRCHUD:SetCustomNumberCrit("armor", LocalPlayer():Armor() < 25);
  end

  GSRCHUD:AddTheme(theme, Color(50, 255, 50), Color(255, 0, 0), spriteData, nil, script); -- Add theme
end
