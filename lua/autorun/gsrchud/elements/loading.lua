...
cvars.AddChangeCallback(HOOK, function(cvar, old, new)
  if not GSRCHUD.loadingPanel then return end
  GSRCHUD.loadingPanel:SetVisible(cvar:GetBool())
end)
...
hook.Add('Think', HOOK, function()
  if not GSRCHUD.isEnabled() or delay > CurTime() or offset >= 1 or not GSRCHUD.config.getLoading() then return end
...
concommand.Add(GSRCHUD.hookname .. '_reset_loading', function(_, _, _)
  ...
  GSRCHUD.loadingPanel:SetupPlayer(panel.player) -- refresh the player model
  delay = CurTime() + DELAY
  offset = 0
end)
